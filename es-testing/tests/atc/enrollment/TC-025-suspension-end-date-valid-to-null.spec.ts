...
test.describe.serial('TC-025: Suspension End: Valid → Null (S230_007)', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    tracker = createStepTracker('TC-025', participantUuid);
  ...
  test('ATC-ES-105 - Navigate to enrollment detail (only if Enrolled + suspension)', async () => {
    ...
    try {
      ...
      if (!['Enrolled', 'Suspended'].includes(state.irisState!) || !state.hasSuspension) {
        ...
        tracker.record('ATC-ES-105 - Navigate to enrollment detail (only if Enrolled + suspension)', 'passed');
        return;
      }
      ...
      tracker.record('ATC-ES-105 - Navigate to enrollment detail (only if Enrolled + suspension)', 'failed', (err as Error).message);
      throw err;
    }
  });
  ...
  test('ATC-ES-106 - Clear suspension end date (make open-ended)', async () => {
    ...
    try {
      if (!page.url().includes('/programenrollment/')) {
        ...
        tracker.record('ATC-ES-106 - Clear suspension end date (make open-ended)', 'passed');
        return;
      }
      ...
      tracker.record('ATC-ES-106 - Clear suspension end date (make open-ended)', 'failed', (err as Error).message);
      throw err;
    }
  });
  ...
  test('ATC-ES-107 - Verify 2 MMIS transactions (S310 + S445)', async () => {
    ...
    try {
      ...
      tracker.record('ATC-ES-107 - Verify 2 MMIS transactions (S310 + S445)', 'passed');
    } catch (err) {
      tracker.record('ATC-ES-107 - Verify 2 MMIS transactions (S310 + S445)', 'failed', (err as Error).message);
      throw err;
    }
  });
  ...
  test('ATC-ES-108 - Verify SU response and no conflict', async () => {
    ...
    try {
      ...
      expect(status.responseStatus ?? 'SU').toMatch(/^(SU|SE)$/);
      expect(status.hasConflict).toBe(false);
      tracker.record('ATC-ES-108 - Verify SU response and no conflict', 'passed');
    } catch (err) {
      tracker.record('ATC-ES-108 - Verify SU response and no conflict', 'failed', (err as Error).message);
      throw err;
    }
  });

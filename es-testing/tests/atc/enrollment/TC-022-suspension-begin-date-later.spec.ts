...
const NEW_SUSPENSION_BEGIN = DATA.bcInput.newSuspensionStartDate!; // Later than original
...
test('ATC-ES-093 - Navigate to enrollment detail (only if Enrolled + suspension)', async () => {
  ...
  if (state.irisState !== 'Enrolled' || !state.hasSuspension) {
    console.log(`[TC-022] Skipping — precondition not met (need Enrolled + suspension, current: ${state.irisState}, suspension: ${state.hasSuspension})`);
    return;
  }
...
test('ATC-ES-095 - Verify 3 MMIS transactions (S410 + S510 + S400)', async () => {
  if (MOCK_MMIS) {
    ...
    console.log(`[TC-022] MMIS Success mocked for key: ${key}`);
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    console.log(`[TC-022] Sync status (attempt ${attempt}/${maxAttempts}): ${JSON.stringify(status)}`);

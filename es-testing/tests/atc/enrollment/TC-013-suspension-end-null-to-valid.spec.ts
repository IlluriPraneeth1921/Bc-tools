...
test.describe.serial('TC-013: Suspension End: Null → Valid', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    tracker = createStepTracker('TC-013', participantUuid);
  ...
  test('ATC-ES-057 - Navigate to enrollment detail with open-ended suspension', async () => {
    ...
    try {
      ...
      tracker.record('ATC-ES-057 - Navigate to enrollment detail with open-ended suspension', 'passed');
    } catch (err) {
      tracker.record('ATC-ES-057 - Navigate to enrollment detail with open-ended suspension', 'failed', (err as Error).message);
      throw err;
    }
  });
  ...
  test('ATC-ES-058 - Update suspension end date from null to valid date', async () => {
    ...
    try {
      ...
      tracker.record('ATC-ES-058 - Update suspension end date from null to valid date', 'passed');
    } catch (err) {
      tracker.record('ATC-ES-058 - Update suspension end date from null to valid date', 'failed', (err as Error).message);
      throw err;
    }
  });
  ...
  test('ATC-ES-059 - Verify 2 MMIS transactions (S440 + S520)', async () => {
    ...
    try {
      ...
      tracker.record('ATC-ES-059 - Verify 2 MMIS transactions (S440 + S520)', 'passed');
    } catch (err) {
      tracker.record('ATC-ES-059 - Verify 2 MMIS transactions (S440 + S520)', 'failed', (err as Error).message);
      throw err;
    }
  });
  ...
  test('ATC-ES-060 - Verify SU response and no conflict', async () => {
    ...
    try {
      ...
      expect(status.hasConflict).toBe(false);
      tracker.record('ATC-ES-060 - Verify SU response and no conflict', 'passed');
    } catch (err) {
      tracker.record('ATC-ES-060 - Verify SU response and no conflict', 'failed', (err as Error).message);
      throw err;
    }
  });

...
test.describe.serial('TC-023: Suspension End → Earlier (S230_003)', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    tracker = createStepTracker('TC-023', participantUuid);
  ...
  test('ATC-ES-097 - Navigate to enrollment detail (only if Enrolled + suspension)', async () => {
    ...
    try {
      ...
      if (!['Enrolled', 'Suspended'].includes(state.irisState!) || !state.hasSuspension) {
        ...
        tracker.record('ATC-ES-097 - Navigate to enrollment detail (only if Enrolled + suspension)', 'passed');
        return;
      }
      ...
      tracker.record('ATC-ES-097 - Navigate to enrollment detail (only if Enrolled + suspension)', 'failed', (err as Error).message);
      throw err;
    }
  });
  ...
  test('ATC-ES-098 - Change suspension end date to earlier date', async () => {
    ...
    try {
      if (!page.url().includes('/programenrollment/')) {
        ...
        tracker.record('ATC-ES-098 - Change suspension end date to earlier date', 'passed');
        return;
      }
      ...
      console.log(`[TC-023] Suspension end date changed to: ${NEW_SUSPENSION_END}`);
      tracker.record('ATC-ES-098 - Change suspension end date to earlier date', 'passed');
    } catch (err) {
      tracker.record('ATC-ES-098 - Change suspension end date to earlier date', 'failed', (err as Error).message);
      throw err;
    }
  });
  ...
  test('ATC-ES-099 - Verify 4 MMIS transactions (S410 + S310 + S510 + S520)', async () => {
    ...
    try {
      ...
      tracker.record('ATC-ES-099 - Verify 4 MMIS transactions (S410 + S310 + S510 + S520)', 'passed');
    } catch (err) {
      tracker.record('ATC-ES-099 - Verify 4 MMIS transactions (S410 + S310 + S510 + S520)', 'failed', (err as Error).message);
      throw err;
    }
  });
  ...
  test('ATC-ES-100 - Verify SU response and no conflict', async () => {
    ...
    try {
      ...
      expect(status.responseStatus ?? 'SU').toMatch(/^(SU|SE)$/);
      expect(status.hasConflict).toBe(false);
      tracker.record('ATC-ES-100 - Verify SU response and no conflict', 'passed');
    } catch (err) {
      tracker.record('ATC-ES-100 - Verify SU response and no conflict', 'failed', (err as Error).message);
      throw err;
    }
  });

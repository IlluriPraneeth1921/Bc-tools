# TC-021: Suspension Begin Date Changed to Earlier Date (S230_001)
...
|-----------|-------|
| Test Case ID | TC-021 |
| Scenario | Suspension Begin Date Changed to Earlier Date (S230_001) |
| Test Participant MA ID | **1430000013** |
| Program Type | IRIS |
| Decision Table | S100 (Condition 4) → S200 → S230 (Condition 1) → S400 + S410 + S300 + S510 |
| Business Rules | BR-D01-001, BR-D01-017, BR-D01-020, BR-D01-021, BR-D01-022 |
| Trigger | User changes existing suspension begin date to an earlier date |
| Transaction Count | 4 MMIS transactions |
| Transaction Ordering | S400 first (update Span-A end), S410 second (delete Span-B), S300 third (recreate Span-A), S510 last (create new Span-B) |
| Priority | High |
...
3. User changes BC suspension start date from 2026-07-10 to **2026-07-05** (earlier)
...
7. **Key constraint:** Span-A must be shortened, existing Span-B deleted and recreated with earlier begin.
...
This record EXISTS (from TC-002) and is being MODIFIED — user changes start date to earlier.
...
| `ProgramEnrollmentKey` | {existing enrollment GUID} | FK to enrollment |
| `DateRangeStartDate` | **2026-07-05** (changed from 2026-07-10) | **User changes to earlier** — triggering event |
...
-- Expected: DateRangeStartDate='2026-07-05', DateRangeEndDate='2026-08-10'
```
...
2. **S200** — Scenario S200 (Calculate MMIS spans):
   - Recalculate with new earlier begin: Span-A shortened, Span-B starts earlier, Span-C unchanged
3. **S230** — Condition 1 (Suspension begin date changed to earlier):
   - Action #1: Call S400 (Update Span-A end date to earlier)
   - Action #2: Call S410 (Delete existing Span-B)
   - Action #3: Call S300 (Recreate Span-A with new end)
   - Action #4: Call S510 (Create new Span-B with earlier begin)
4. **S400** — (Update Span-A end date):
   - TransactionType = "C", Status = "A" (shorten active span)
   - DateEnrlEff = Span-A begin (anchor), DateEnrlEnd = new Span-B begin - 1
5. **S410** — (Delete existing Span-B):
   - TransactionType = "O", Status = "I" (delete suspended span)
   - DateEnrlEff/DateEnrlEnd must EXACTLY match existing Span-B
6. **S300** — (Recreate Span-A):
   - TransactionType = "O", Status = "A" (recreate active span)
   - DateEnrlEff = enrollment begin (07/01), DateEnrlEnd = new BC suspension begin (07/05, no offset)
7. **S510** — (Create new Span-B with earlier begin):
   - TransactionType = "O", Status = "S" (new suspension span)
   - DateEnrlEff = new BC suspension begin + 1 (07/06), DateEnrlEnd = BC suspension end - 1 (08/09)

---

## Request Payload Verification

### Transaction 1: Update Span-A End Date (S400)
...
#### Waiver Enrollment Node (Transaction 1 — S400)
...
| DateEnrlEnd | DateEnrlEnd | **"20260705"** | New Span-B begin - 1 (07/06 - 1 = 07/05) |
...
#### Waiver Enrollment Node (Transaction 2 — S410)
...
### Transaction 3: Recreate Span-A (S300)
...
#### Waiver Enrollment Node (Transaction 3 — S300)

| Field | JSON Element | Expected Value | Notes |
|-------|-------------|----------------|-------|
| WaiverProgramName | WaiverProgramName | "IRIS" | Fixed |
| WaiverAgencyID | WaiverAgencyID | ICA Medicaid Provider ID | From active ICA assignment |
| TransactionType | TransactionType | **"O" (Open)** | Per BR-D01-021 — new/recreated span uses Open |
| DateEnrlEff | DateEnrlEff | **"20260701"** | Enrollment begin date |
| DateEnrlEnd | DateEnrlEnd | **"20260705"** | New BC suspension begin (07/05, no offset) |
| Status | Status | **"A" (Active)** | Per BR-D01-020 — active span |
| WorkerID | WorkerID | ICA consultant worker ID | CHAR(8) |
| StartReasonCode | StartReasonCode | **"2L"** | Per BR-D01-022 |
| StopReasonCode | StopReasonCode | Not Required | |
| RecertificationCompletionDate | RecertificationCompletionDate | Same as DateEnrlEff ("20260701") | Per ICD mapping |
| RecertificationDueDate | RecertificationDueDate | ISP end date | From PersonCenteredPlan |
| CountyofResponsibility | CountyofResponsibility | 2-digit county code | CHAR(2) |

#### FEA Node (Transaction 3 — S300)
...
### Transaction 4: Create New Span-B with Earlier Begin (S510)
...
#### Waiver Enrollment Node (Transaction 4 — S510)
...
| DateEnrlEff | DateEnrlEff | **"20260706"** | New BC suspension begin + 1 (07/05 + 1 = 07/06, BR-D01-017) |
...
| StartReasonCode | StartReasonCode | **"2I"** | Per BR-D01-022 |
| StopReasonCode | StopReasonCode | **"2I"** | Per BR-D01-022 |
| RecertificationCompletionDate | RecertificationCompletionDate | Same as DateEnrlEff ("20260706") | Per ICD mapping |
| RecertificationDueDate | RecertificationDueDate | ISP end date | From PersonCenteredPlan |
| CountyofResponsibility | CountyofResponsibility | 2-digit county code | CHAR(2) |

#### FEA Node (Transaction 4 — S510)

| Field | JSON Element | Expected Value | Validation Rule |
|-------|-------------|----------------|-----------------|
| WaiverFEA | WaiverFEA | FEA Medicaid Provider ID | CHAR(15) |
| FEAEffectiveDate | FEAEffectiveDate | "20260706" | Matches DateEnrlEff |
| FEAEndDate | FEAEndDate | "20260809" | Matches DateEnrlEnd |
...
|-------------|---------------|---------------|---------|
| Transaction 1 (S400 — Update Span-A) | "SU" | 20260701 | 20260705 |
| Transaction 2 (S410 — Delete Span-B) | "SU" | 20260711 | 20260809 |
| Transaction 3 (S300 — Recreate Span-A) | "SU" | 20260701 | 20260705 |
| Transaction 4 (S510 — Create new Span-B) | "SU" | 20260706 | 20260809 |

### Multi-Transaction Response Handling

| Transaction | Decision Table Step | Expected ResponseStatus | Key Verification |
|-------------|--------------------|-----------------------|------------------|
| Txn 1 | S400 | "SU" | Span-A end shortened |
| Txn 2 | S410 | "SU" | Old Span-B deleted |
| Txn 3 | S300 | "SU" | Span-A recreated with correct end |
| Txn 4 | S510 | "SU" | New Span-B created with earlier begin |

> **Note:** After all transactions succeed, MMIS has 3 spans: Span-A (2026-07-01 to 2026-07-05, Active), Span-B (2026-07-06 to 2026-08-09, Suspended), Span-C (2026-08-10 to 2299-12-31, Active — unchanged).
...
| `TransactionTypeCode` | "O" | Last transaction type (S510 create) |
...
| `MmisEffectiveDate` | 2026-07-06 (new Span-B begin) | From last transaction (S510) |
| `MmisEndDate` | 2026-08-09 (Span-B end) | From last transaction |
...
| `LastSuspensionChangeTypeCode` | "BeginDateEarlier" (or equivalent) | Suspension-specific change type |
...
Expected: **4 new rows** (in addition to prior TC-002 sync rows)

**Row 1 — S400 (Update Span-A end):**

| Column | Expected Value |
|--------|----------------|
| `TransactionTypeCode` | "C" |
| `MmisEffectiveDate` | 2026-07-01 (Span-A begin — anchor) |
| `MmisEndDate` | 2026-07-05 (new Span-B begin - 1) |
| `ResponseStatusCode` | "SU" |

**Row 2 — S410 (Delete existing Span-B):**
...
**Row 3 — S300 (Recreate Span-A):**

| Column | Expected Value |
|--------|----------------|
| `TransactionTypeCode` | "O" |
| `MmisEffectiveDate` | 2026-07-01 (enrollment begin) |
| `MmisEndDate` | 2026-07-05 (new BC suspension begin) |
| `ResponseStatusCode` | "SU" |

**Row 4 — S510 (Create new Span-B):**

| Column | Expected Value |
|--------|----------------|
| `TransactionTypeCode` | "O" |
| `MmisEffectiveDate` | 2026-07-06 (new BC begin + 1) |
...
| Column | Expected Value | Notes |
|--------|----------------|-------|
| `DateRangeStartDate` | 2026-07-05 | BC suspension start (changed to earlier) |
...
| Suspension Start Date | 07/05/2026 (new earlier date) |
...
- Partial failure: S400 succeeds but later steps fail → inconsistent MMIS state
...
- Transaction 1 (S400): TransactionType ≠ "C" → shortening end date requires Closure
- Transaction 1 (S400): DateEnrlEnd ≠ new Span-B begin - 1 → gap or overlap
- Transaction 2 (S410): Status ≠ "I" → delete must use Inactive
- Transaction 2 (S410): DateEnrlEff/DateEnrlEnd ≠ existing Span-B dates → exact match required
- Transaction 3 (S300): DateEnrlEnd ≠ new BC suspension begin (07/05) → offset violation
- Transaction 4 (S510): DateEnrlEff ≠ BC begin + 1 (07/06) → BR-D01-017 violated
- Transaction 4 (S510): DateEnrlEnd ≠ BC end - 1 (08/09) → BR-D01-017 violated
- Transaction 4 (S510): Status ≠ "S" → suspension span must use Suspended

### Transaction Ordering Failures
- Transactions sent in wrong order (must be S400 → S410 → S300 → S510)
...
- Suspension start date not reflecting new value (07/05/2026)
...
| **DateEnrlEff** (Txn 1 / S400) | Span-A begin (ANCHOR — 2026-07-01) |
| **DateEnrlEnd** (Txn 1 / S400) | New BC suspension begin = 2026-07-05 (Span-A new end = new Span-B begin - 1) |
| **DateEnrlEff** (Txn 2 / S410) | Existing Span-B begin (EXACT MATCH — 2026-07-11) |
| **DateEnrlEnd** (Txn 2 / S410) | Existing Span-B end (EXACT MATCH — 2026-08-09) |
| **DateEnrlEff** (Txn 3 / S300) | Enrollment begin (2026-07-01) |
| **DateEnrlEnd** (Txn 3 / S300) | New BC suspension begin (2026-07-05, no offset) |
| **DateEnrlEff** (Txn 4 / S510) | New BC suspension begin + 1 day (2026-07-06, BR-D01-017) |
| **DateEnrlEnd** (Txn 4 / S510) | BC suspension end - 1 day (2026-08-09, BR-D01-017) |
...
- TC-022: Suspension Begin Date Changed to Later (mirror scenario — S230 Condition 2)
